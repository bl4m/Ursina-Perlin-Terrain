...
class Generate_Terrain:
    def __init__(self,xSize=0,zSize=0,freq=0,amp=0,octaves=0,seed=0,colorise=False):
        self.freq = freq
        self.amp = amp
        self.xSize = zSize
        self.zSize = xSize
...
if __name__ == "__main__":
    #create app
    app = Ursina()
    #initialise generate_terrain comand
    world = Generate_Terrain(100,#xsize
                             100,#ysize
                             20,#frequency(how frequently new mountains generate)
                             10,#amplitude(how high or low the mountains will be)
                             1,#octaves(how many mountains there will be)
                             987#seed(random number)
                            )
